 ...
 * from MultiPlayer mode: the main differences are in the management
...
public class SinglePlayerController extends Controller {
     ...
     * This method places the chosen card at the first position (0)
     ...
     * {@code fillScoreBoard} to find the {@link RoundTrack} at the second
     * position (1) in the array of players. These methods are executed right after a
     * correct {@link PrivateObjectiveCard} selection.
     *
     * @param message The message generated by the view that
     *                contains the name of the card chosen.
     */
    private void selectPrivateObjective(ViewMessage message) {

        SelectCard selectCard = (SelectCard) message;
        String name = selectCard.getName();
        Player player = getGame().getPlayers().get(0);

        for (PrivateObjectiveCard card : getGame().getPlayers().get(0).getCards()) {
            if (card.getName().equals(name)) {
                PrivateObjectiveCard[] newCards = new PrivateObjectiveCard[]{card};
        ...
        selectCard.getView().showError("The chosen private objective card is not available!");
    }
     ...
     * The toolCard won't be available in Single Player mode in various
     ...
     * it starts disconnectedTimer and maintains the actual {@link Game} instance still available
