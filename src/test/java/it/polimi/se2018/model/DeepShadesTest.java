...
/**
 * Unit tests for DeepShades class.
 */
public class DeepShadesTest {
    ...
    public void testMultipleInstantiation() {
        DeepShades firstInstance = DeepShades.getInstance();
        DeepShades secondInstance = DeepShades.getInstance();
     ...
     * the value of which is 5 and 6.</p>
     */
    @Test
    public void testSimpleScoreCalculation() {
        ...
        try {
            for(int i = 0; i < grid[0].length; ++i)
                grid[0][i].place(dice[5]);
            for(int i = 0; i < grid[0].length - 2; ++i)
                grid[1][i].place(dice[4]);
        ...
        int actualScore = DeepShades.getInstance().getScore(grid);
